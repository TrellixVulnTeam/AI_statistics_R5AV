...
def main(search_day_from, search_day_to):
    ############ 공통 변수 ############
    #today = datetime.today().strftime("%Y-%m-%d")
    search_day_list = []

    d1 = date(int(search_day_from[0:4]), int(search_day_from[5:7]), int(search_day_from[8:10]))
    d2 = date(int(search_day_to[0:4]), int(search_day_to[5:7]), int(search_day_to[8:10]))
    delta = d2 - d1

    for i in range(delta.days + 1):
        search_day_list.append(str(d1 + timedelta(days=i)))
    ...
    ###########################################      ID-날짜 시트      #########################################################
    for root, dirs, files in os.walk("\\\\192.168.0.192/DL/"):
    # for root, dirs, files in os.walk("./DL"):
        ...
        if len(files) > 0:
            ...
            if root_split[-2].split("_")[-1] not in search_day_list:
                continue
    ...
    ###########################################      랭킹-날짜 시트      #########################################################
    for root, dirs, files in os.walk("\\\\192.168.0.192/DC/"):
    # for root, dirs, files in os.walk("./DC/"):
        ...
        if len(files) > 0:
            root_split = root.split("\\")
            if root_split[-3].split("_")[-1] not in search_day_list:
                continue
            for file_name in files:
...
try:
    search_day_from = input("통계 낼 시작날짜 입력 (ex.2020-10-23)")
    search_day_to = input("통계 낼 종료날짜 입력 (ex.2020-10-24)")
    main(search_day_from, search_day_to)

...
filtering_folder = ["삭제검토","에러파일","한쌍이안됨"]

def category_counting(path, start_time):
    jpg_count = 0
    json_count = 0

    for root, dirs, files in os.walk(path):
    # for root, dirs, files in os.walk("\\\\192.168.0.192/DL/one_folder"):
    ...
    write_excel(sub_category_dict,json_count,jpg_count, start_time, path)
    print(sub_category_dict)
    print(main_category_dict)

def write_excel(sub_category_dict, json_count, jpg_count, start_time, path):
    finish_time = datetime.datetime.now()
    one_folder_wb = openpyxl.load_workbook('one_folder_form.xlsx')
    one_folder_total_sheet = one_folder_wb['one_folder']
    idx = 5
    ...
    one_folder_total_sheet['C99'] = start_time.strftime('%Y-%m-%d %H:%M:%S')
    one_folder_total_sheet['F99'] = finish_time.strftime('%Y-%m-%d %H:%M:%S')

    one_folder_total_sheet['C100'] = path

    if not (os.path.isdir("./output")):
        os.makedirs(os.path.join("./output"))
    one_folder_wb.save('./output/one_folder.xlsx')
...
try:
    ...
    category_counting(path, start_time)

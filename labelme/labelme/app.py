...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        createBlurMode = action(
            self.tr("Create Blur"),
            lambda: self.toggleDrawMode(False, createMode="blur"),
            # shortcuts["create_rectangle"],
            "objects",
            self.tr("Start drawing blur"),
            enabled=False,
        )
    ...
    def setClean(self):
        ...
        self.actions.createBlurMode.setEnabled(True)
    ...
    def toggleDrawMode(self, edit=True, createMode="polygon"):
        ...
        if edit:
            ...
            if createMode == "polygon":
                ...
                self.actions.createPointMode.setEnabled(True)
                self.actions.createLineStripMode.setEnabled(True)
            elif createMode == "blur":
                self.actions.createMode.setEnabled(True)
                self.actions.createRectangleMode.setEnabled(True)
                self.actions.createBlurMode.setEnabled(False)
                self.actions.createCircleMode.setEnabled(True)
                self.actions.createLineMode.setEnabled(True)
                self.actions.createPointMode.setEnabled(True)
                self.actions.createLineStripMode.setEnabled(True)
    ...
    def saveLabels(self, filename):
        ...
        shapes = [format_shape(item.shape()) for item in self.labelList if item.shape().shape_type != "blur"]
        ...
        try:
            for item in self.labelList:
                if item.shape().shape_type == "blur":
                    has_blur_flag = True
                    break
            if has_blur_flag:
                imagePath = osp.relpath(self.imagePath, osp.dirname(filename))
                imagePath = os.path.splitext(imagePath)[0] + "_blur.jpg"

                byte_array = QtCore.QByteArray()
                buffer = QtCore.QBuffer(byte_array)
                buffer.open(QtCore.QIODevice.WriteOnly)
                blur_image = self.image
                blur_image.save(buffer, "jpg", quality=100)

                self.imageData = byte_array.data()
            else :
                imagePath = osp.relpath(self.imagePath, osp.dirname(filename))
    ...
    def newShape(self, createMode):
        ...
        if self._config["display_label_popup"] or not text:
            ...
            text, flags, group_id = self.labelDialog.popUp(text, createMode=createMode)
            if not text:
    ...
    def loadFile(self, filename=None):
        ...
        filename = self.lastOpenDir + "/" + filename
    ...
    def saveFileDialog(self):
        ...
        if has_blur_flag:
            default_labelfile_name = os.path.splitext(default_labelfile_name)[0] + "_blur.json"
    ...
    def _saveFile(self, filename):
        ...
        if blur_save_flag:
            ...
            self.image.save(file_name + ".jpg", "JPEG")
